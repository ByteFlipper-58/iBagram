                    ...
                    when (val result = togglePeerNotificationsUseCase(event.type, event.enabled)) {
                        is Result.Success -> {
                            ...
                            val updated = when (event.type) {
                                org.telegram.messenger.feature.notifications.domain.model.NotificationPeerType.PRIVATE_CHATS ->
                                    current.copy(privateChatsEnabled = event.enabled)
                                org.telegram.messenger.feature.notifications.domain.model.NotificationPeerType.GROUPS ->
                                    current.copy(groupsEnabled = event.enabled)
                                org.telegram.messenger.feature.notifications.domain.model.NotificationPeerType.CHANNELS ->
                                    current.copy(channelsEnabled = event.enabled)
                                org.telegram.messenger.feature.notifications.domain.model.NotificationPeerType.STORIES ->
                                    current.copy(storiesEnabled = event.enabled)
                                org.telegram.messenger.feature.notifications.domain.model.NotificationPeerType.REACTIONS_MESSAGES ->
                                    current.copy(reactionsMessagesEnabled = event.enabled)
                                org.telegram.messenger.feature.notifications.domain.model.NotificationPeerType.REACTIONS_STORIES ->

...
import org.telegram.messenger.feature.media.audioplayer.domain.model.EqualizerBand
import org.telegram.messenger.feature.media.audioplayer.domain.model.EqualizerState
import org.telegram.messenger.feature.media.audioplayer.domain.model.RepeatMode

/**
 * Local data source managing active audio player state, equalizer presets,
 * playlist queues, and audio output routes.
 */
class AudioPlayerLocalDataSource(
    ...
    fun observePlaybackState(): Flow<AudioPlaybackState> = _state.asStateFlow()

    fun getPlaybackState(): AudioPlaybackState = _state.value
    ...
    suspend fun setEqualizerBandGain(
        bandIndex: Int,
        gainMilliBels: Int
    ) {
        _state.update { current ->

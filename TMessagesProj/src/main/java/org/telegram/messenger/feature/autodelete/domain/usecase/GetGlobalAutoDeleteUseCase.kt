...
class GetGlobalAutoDeleteUseCase(
    private val repository: AutoDeleteRepository
) {
    suspend operator fun invoke(forceRefresh: Boolean = false): Result<AutoDeleteTtlModel> {
        return repository.getGlobalAutoDelete(forceRefresh)
    }
}

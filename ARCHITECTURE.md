...
- **Decision:** Extract chat folders and dialog filters logic into an isolated, dedicated `DialogFiltersController.kt` written purely in Kotlin extending `BaseController`, instantiated via per-account singleton (`DialogFiltersController.getInstance(account)`). Connect `DialogFiltersController` directly to modern `FoldersRepository` via strangler hooks. In `MessagesController.java`, delegate all dialog filter methods (`loadRemoteFilters`, `loadSuggestedFilters`, `selectDialogFilter`, `onFilterUpdate`, `addFilter`, `removeFilter`, `lockFiltersInternal`) to `getDialogFiltersController()`, shrinking legacy lines while maintaining 100% backward compatibility for all upstream Telegram UI callers.
- **Consequences:** `MessagesController.java` is reduced by 138 lines; chat folder operations are modularized into modern Kotlin and testable in isolation; all 100% upstream callers continue to compile and function without modification.

### ADR 224: Extraction of EmojiStatusController from MessagesController (Batch 4.3)
- **Context:** In `MessagesController.java`, emoji status updates (`updateEmojiStatus`, `updateEmojiStatusUntil`, `updateEmojiStatusUntilUpdate`, `emojiStatusCollectibleFromGift`), emoji status expiration timeouts, and collectibles logic were embedded within the monolithic controller along with mutable fields (`emojiStatusUntilValues`, `recentEmojiStatusUpdateRunnable*`).
- **Decision:** Extract emoji status and collectible logic into a dedicated, isolated `EmojiStatusController.kt` written purely in Kotlin extending `BaseController`, instantiated via per-account singleton (`EmojiStatusController.getInstance(account)`). Wire `EmojiStatusController` directly to `ReactionsRepository` via strangler hook `reactionsRepository`. In `MessagesController.java`, eliminate duplicate state fields (`emojiStatusUntilValues`, `recentEmojiStatusUpdateRunnable*`), delegate all emoji status methods directly to `getEmojiStatusController()`, and expose `getEmojiStatusController()` on `AccountInstance.java` and `MessagesController.java`.
- **Consequences:** `MessagesController.java` is reduced by another 93 lines of legacy code; emoji status handling is modularized into 100% Kotlin with thread-safe ConcurrentHashMap tracking and full Java interoperability; 100% upstream compatibility is preserved.

### ADR 225: Extraction of QuickRepliesController in Kotlin and Strangler Hooks (Batch 4.4)
- **Context:** In `MessagesController.java`, quick replies query timestamps (`lastQuickReplyServerQueryTime`), cache invalidation logic, and reload rate-limiting checks were embedded inside the monolithic controller, causing coupling between `MessagesController` and `org.telegram.ui.Business.QuickRepliesController`.
- **Decision:** Introduce a dedicated `org.telegram.messenger.QuickRepliesController.kt` written in 100% Kotlin extending `BaseController`, providing thread-safe query time tracking (`LongSparseArray`), `shouldReloadMessages`, `recordServerQueryTime`, `clearQueryTimes`, and direct strangler hook to `QuickRepliesRepository`. Expose `getQuickRepliesController()` on `AccountInstance.java` and `MessagesController.java`. In `MessagesController.java`, remove `lastQuickReplyServerQueryTime` field, delegate query time tracking and checks to `getQuickRepliesController()`, and qualify UI quick replies controller references. Add strangler hooks (`getQuickRepliesRepository()`, `getRepository()`) to `org.telegram.ui.Business.QuickRepliesController.java`.
- **Consequences:** Responsibilities around quick reply server timestamps and limits are extracted into clean Kotlin; `QuickRepliesRepository` is accessible across both messenger core and UI controllers; JVM unit tests pass deterministically with JVM clock fallback; 100% upstream compatibility is maintained.

### ADR 226: Media Album Queue and Batch Dispatching Strangler Hooks in SendMessagesHelper (Batch 4.5)
- **Context:** In Telegram Android, media albums (multi-photo, multi-video groups, paid media packs) are prepared, queued, and dispatched inside `SendMessagesHelper.java` (~12,400 lines) using `DelayedMessage` with `type = 4` and group ID keys (`"group_" + groupId`). The modern `SendMessagesRepository` lacked visibility into active album queues and batch dispatching status.
...
### ADR 228: Extraction of HistoryImportController in Kotlin from SendMessagesHelper (Batch 4.7)
- **Context:** Third-party chat history importing (from WhatsApp, Line, KakaoTalk) and custom sticker pack importing were embedded directly inside the monolithic `SendMessagesHelper.java` (~12,300 lines). The preparation, cache unzipping, size validation, upload tracking maps (`importingHistoryFiles`, `importingHistoryMap`, `importingStickersFiles`, `importingStickersMap`), and life-cycle listeners created significant coupling and bloat inside `SendMessagesHelper`.
- **Decision:** Extract chat history and sticker pack import operations into a dedicated `org.telegram.messenger.HistoryImportController.kt` written in 100% Kotlin extending `BaseController`. Expose per-account singleton accessors on `AccountInstance.java` and `MessagesController.java`. Expose factory methods `createImportingHistory()` and `createImportingStickers()` on `SendMessagesHelper.java` to instantiate inner tracker classes cleanly. Delegate `prepareImportHistory`, `prepareImportStickers`, `getImportingHistory`, `getImportingStickers`, `isImportingHistory`, and `isImportingStickers` from `SendMessagesHelper.java` to `HistoryImportController.kt`, eliminating 187 lines of legacy duplicate spaghetti code.
- **Consequences:** `SendMessagesHelper.java` is reduced by 187 lines of legacy code; history and sticker pack imports are modularized into modern Kotlin with full JVM unit test coverage (`HistoryImportControllerTest.kt`); 100% backward compatibility is preserved for all upstream callers.
### ADR 229: Extraction of TranscribeAudioController in Kotlin from MessagesController (Batch 4.8)
- **Context:** In `MessagesController.java`, audio transcription trial quotas (`transcribeAudioTrialWeeklyNumber`, `transcribeAudioTrialCurrentNumber`, `transcribeAudioTrialDurationMax`, `transcribeAudioTrialCooldownUntil`), group transcription level requirements (`groupTranscribeLevelMin`), button interaction counts (`transcribeButtonPressed`), and timer notification scheduling (`scheduleTranscriptionUpdate`) were intertwined with general message and chat state.
- **Decision:** Extract transcription trial limits, cooldown checks, quota tracking, and button interaction thresholds into a dedicated `org.telegram.messenger.TranscribeAudioController.kt` written in 100% Kotlin extending `BaseController`. Expose per-account singleton accessors on `AccountInstance.java` and `MessagesController.java`. In `MessagesController.java`, delegate `didPressTranscribeButtonEnough`, `pressTranscribeButton`, `updateTranscribeAudioTrialCurrentNumber`, `updateTranscribeAudioTrialCooldownUntil`, and `scheduleTranscriptionUpdate` to `getTranscribeAudioController()`, while keeping public fields synchronized for zero-overhead backward compatibility with UI components (`TranscribeButton.java`, `ChatActivity.java`).
- **Consequences:** Transcription limits and button interaction rules are modularized into modern, testable Kotlin; `TranscribeAudioControllerTest.kt` provides 100% deterministic JVM unit test verification without triggering Android UI or DisplayMetrics dependencies; full upstream compatibility is preserved.

### ADR 230: Extraction of SponsoredMessagesController in Kotlin from MessagesController (Batch 4.9)
- **Context:** In `MessagesController.java`, sponsored messages (ad channels, bot promotions, sponsored links), per-dialog caching (`sponsoredMessages`), 5-minute TTL invalidation, channel restrict levels (`channelRestrictSponsoredLevelMin`), MTProto RPC calls (`TL_messages_getSponsoredMessages`, `TL_messages_toggleSponsoredMessages`), and read markers (`markSponsoredAsRead`) were embedded directly inside the monolithic controller, adding ~250 lines of legacy clutter and coupling to UI screens (`ChatActivity.java`, `ChannelAdminLogActivity.java`).
- **Decision:** Extract sponsored messages querying, caching, TTL checks, and ad toggling into a dedicated `org.telegram.messenger.SponsoredMessagesController.kt` written in 100% Kotlin extending `BaseController`. Expose per-account singleton accessors on `AccountInstance.java` and `MessagesController.java`. Make `MessagesController.SponsoredMessagesInfo` a `public static class` to maintain 100% source and binary compatibility for legacy callers. In `MessagesController.java`, delegate `getSponsoredMessages`, `markSponsoredAsRead`, `disableAds`, and `isSponsoredDisabled` to `getSponsoredMessagesController()`, and synchronize `channelRestrictSponsoredLevelMin` and `sponsoredLinksInappAllow`.
- **Consequences:** `MessagesController.java` is relieved of sponsored message caching and RPC handling; sponsored messages logic is modularized into modern Kotlin with full JVM unit test coverage (`SponsoredMessagesControllerTest.kt`); 100% backward compatibility is preserved for all UI components.

### ADR 231: Extraction of BlockedPeersController in Kotlin from MessagesController (Batch 4.9)
- **Context:** In `MessagesController.java`, blocked peers tracking (`blockePeers` `LongSparseIntArray`), pagination offsets (`totalBlockedCount`, `blockedEndReached`), MTProto RPCs (`contacts.getBlocked`, `contacts.block`, `contacts.unblock`), and 4 duplicate 12-line code blocks updating peer blocked status across full users, full chats, and user info updates were tangled inside the monolithic controller. UI screens (`PrivacyUsersActivity.java`, `PrivacySettingsActivity.java`) directly accessed `MessagesController.blockePeers`.
- **Decision:** Extract blocked peer state, pagination counters, MTProto blocking/unblocking RPCs, and reactive status propagation into a dedicated `org.telegram.messenger.BlockedPeersController.kt` written in 100% Kotlin extending `BaseController`. Expose per-account singleton accessors on `AccountInstance.java` and `MessagesController.java`. Share the exact instance of `blockePeers` with `MessagesController.blockePeers = getBlockedPeersController().blockePeers;` to guarantee zero-divergence compatibility with existing UI callers. Consolidate repetitive peer blocked update logic into `onPeerBlockedChanged(peerId, blocked)`.
- **Consequences:** Monolithic `MessagesController.java` is reduced by ~220 lines of repetitive blocking logic; blocked peers state is safely modularized into 100% Kotlin with full JVM unit test coverage (`BlockedPeersControllerTest.kt`); all legacy UI callers (`PrivacyUsersActivity`, `PrivacySettingsActivity`, etc.) continue working seamlessly without any code changes.
